...
from mmdet3d.core import Box3DMode, bbox3d2result, merge_aug_bboxes_3d, show_result
...
class MVXTwoStageDetectorMultiCam(Base3DDetector):
    ...
    def __init__(
        ...
        super(MVXTwoStageDetectorMultiCam, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        ...
        super(MVXTwoStageDetectorMultiCam, self).init_weights(pretrained)
    ...
    def extract_img_feat(self, img, img_metas):
        ...
        For the case of multi view images we process each camera sequentially
        The result is either None (no image path) or list of features per camera
        """
        if self.with_img_backbone and img is not None:
            ...
            for img_meta in img_metas:
                ...
                img_feats = []
                for i in range(N):
                    img_feats_c = self.img_backbone(img[:, i, :, :, :])
                    img_feats.append(img_feats_c)
        else:
            return None
        if self.with_img_neck:
            # sequentially process each camera
            for camera_idx in range(N):
                img_feats_c = self.img_neck(img_feats[camera_idx])
                # update the features
                img_feats[camera_idx] = img_feats_c
        return img_feats

    def extract_pts_feat(self, pts, img_feats, img_metas):
        """
        Extract features of points.
        pts (list): list of batch size containing the points.
        img_feats: list of camera amount containing Levels x B x feats features
        ...
        **kwargs,
        ...
        if self.with_img_rpn:
            ...
            rpn_loss_inputs = rpn_outs + \
                (gt_bboxes, img_metas, self.train_cfg.img_rpn)
