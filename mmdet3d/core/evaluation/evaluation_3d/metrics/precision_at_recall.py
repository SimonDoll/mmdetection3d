...
class PrecisionAtRecall(Plottable2dClassMetric):
    ...
    def compute(self, matching_results, data=None):
        ...
        decisions_per_class = self.compute_decisions(
            matching_results, self._similarity_threshold, return_idxs=True
        )

        precisions_at_recalls = {class_id: None for class_id in matching_results.keys()}
        ...
        for class_id in matching_results.keys():
            ...
            precisions = torch.div(tp_cumsum, tp_cumsum + fp_cumsum + self.EPSILON)
