...
class Basemetric(ABC):
    ...
    A metric is responsible for producing a metric result of any kind, subclasses refine this.
    Beside the evaluate interface metrics may implement a compute function that should only be used by other metrics.
    This allows for reusing computation results without the need to use the MetricResult interface
    TODO implement pipeline order
    """
    ...
    def evaluate(self, matching_results, data=None):
        ...
        Metrics are not allowed to change the matching results (as they are shared by all metrics)
        At the moment there is no check for this as it would be runtime inefficient
        Metrics are allowed to permute the matching result list per frame e.g.:
    ...
    @staticmethod
    def compute_decisions(matching_results, similarity_treshold, return_idxs=False):
